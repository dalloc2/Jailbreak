...
local library = {design = getgenv().design == "kali" and "kali" or "uwuware", tabs = {}, draggable = true, flags = {}, title = "dalloc - Jailbreak", open = false, popup = nil, instances = {}, connections = {}, options = {}, notifications = {}, tabSize = 0, theme = {}, foldername = "cheatx_cnfgs", fileext = ".txt"}
...
getgenv().library.InitSettings = function() --beautiful code
  local SettingsTab = getgenv().library:AddTab("Settings"); 
local SettingsColumn = SettingsTab:AddColumn(); 
local SettingsColumn2 = SettingsTab:AddColumn(); 
local SettingSection = SettingsColumn:AddSection("Menu"); 
local ConfigSection = SettingsColumn2:AddSection("Configs");
local Warning = getgenv().library:AddWarning({type = "confirm"});
...
SettingSection:AddColor({text = "Accent Color", flag = "Menu Accent Color", color = Color3.new(0.599623620510101318359375, 0.447115242481231689453125, 0.97174417972564697265625), callback = function(color)
    if getgenv().library.currentTab then
        getgenv().library.currentTab.button.TextColor3 = color;
    end
    for i,v in pairs(getgenv().library.theme) do
...
local back = SettingSection:AddList({text = "Background", max = 4, flag = "background", values = {"Floral", "Flowers", "Circles", "Hearts"}, value = "Floral", callback = function(v)
    if getgenv().library.main then
        getgenv().library.main.Image = backgroundlist[v];
    end
end});
...
back:AddColor({flag = "backgroundcolor", color = Color3.new(), callback = function(color)
    if getgenv().library.main then
        getgenv().library.main.ImageColor3 = color;
    end
end, trans = 1, calltrans = function(trans)
    if getgenv().library.main then
        getgenv().library.main.ImageTransparency = 1 - trans;
    end
end});
...
SettingSection:AddSlider({text = "Tile Size", min = 50, max = 500, value = 50, callback = function(size)
    if getgenv().library.main then
        getgenv().library.main.TileSize = UDim2.new(0, size, 0, size);
    end
end});

-- [Discord Button]
SettingSection:AddButton({text = "Discord", callback = getgenv().DiscordFunction or function() end});

-- [Config Box]
ConfigSection:AddBox({text = "Config Name", skipflag = true});

-- [Config List]
ConfigSection:AddList({text = "Configs", skipflag = true, value = "", flag = "Config List", values = getgenv().library:GetConfigs()});

-- [Create Button]
ConfigSection:AddButton({text = "Create", callback = function()
    getgenv().library:GetConfigs();
    writefile(getgenv().library.foldername .. "/" .. getgenv().library.flags["Config Name"] .. getgenv().library.fileext, "{}");
    getgenv().library.options["Config List"]:AddValue(getgenv().library.flags["Config Name"]);
end});

-- [Save Button]
ConfigSection:AddButton({text = "Save", callback = function()
    local r, g, b = getgenv().library.round(getgenv().library.flags["Menu Accent Color"]);
    Warning.text = "Are you sure you want to save the current settings to config <font color='rgb(" .. r .. "," .. g .. "," .. b .. ")'>" .. getgenv().library.flags["Config List"] .. "</font>?";
    if Warning:Show() then
        getgenv().library:SaveConfig(getgenv().library.flags["Config List"]);
    end
end});
...
ConfigSection:AddButton({text = "Load", callback = function()
    local r, g, b = getgenv().library.round(getgenv().library.flags["Menu Accent Color"]);
    Warning.text = "Are you sure you want to load config <font color='rgb(" .. r .. "," .. g .. "," .. b .. ")'>" .. getgenv().library.flags["Config List"] .. "</font>?";
    if Warning:Show() then
        getgenv().library:LoadConfig(getgenv().library.flags["Config List"]);
    end
end});
...
ConfigSection:AddButton({text = "Delete", callback = function()
    local r, g, b = getgenv().library.round(getgenv().library.flags["Menu Accent Color"]);
    Warning.text = "Are you sure you want to delete config <font color='rgb(" .. r .. "," .. g .. "," .. b .. ")'>" .. getgenv().library.flags["Config List"] .. "</font>?";
    if Warning:Show() then
        local config = getgenv().library.flags["Config List"];
        if table.find(getgenv().library:GetConfigs(), config) and isfile(getgenv().library.foldername .. "/" .. config .. getgenv().library.fileext) then
            getgenv().library.options["Config List"]:RemoveValue(config);
            delfile(getgenv().library.foldername .. "/" .. config .. getgenv().library.fileext);
        end
    end
end});
end
